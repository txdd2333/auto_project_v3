 ...
 *
 * 使用方法：
 *   node linux_scripts/scripts/create-super-admin.js <email> <password>
 *
 * 示例：
...
async function createSuperAdmin() {
  ...
  if (!email || !password) {
    ...
    console.error('使用方法：');
    console.error('  node linux_scripts/scripts/create-super-admin.js <email> <password>\n');
    console.error('示例：');
    console.error('  node linux_scripts/scripts/create-super-admin.js admin@company.com SecurePass123\n');
    process.exit(1);
  }
  ...
  try {
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error('\n⚠️  数据表不存在，请先初始化数据库：');
      console.error('   mysql -h <host> -P <port> -u <user> -p <database> < linux_scripts/scripts/init-database.sql\n');
    }
